...
### Group `label` is not displayed when `OptGroup` is opened and `fieldNames` are customized.

After setting `filedNames`, the name of the `label` field in `OptGroup'`is also changed to the value of the `label` field in 'fieldNames'
